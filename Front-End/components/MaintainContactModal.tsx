...
const MaintainContactModal = ({
  ...
  useEffect(() => {
    if (action === "update" && contactEmail !== email) {
      let existingContact = contacts.find(
        (contact) => contact.email === contactEmail
      );

      originalEmail.current = existingContact!.email;

      setName(existingContact!.name);
      setEmail(existingContact!.email);
      setPhone(existingContact!.phoneNumber);
      setSelectedImage(existingContact!.avatar);
  ...
  const updateContact = async (event: MouseEvent<HTMLButtonElement>) => {
    let button = event.currentTarget;
    button.disabled = true;
    let userEmail = session?.user?.email;
    ...
    try {
      const response = await axios.put(
        "/api/updateContact?originalEmail=" + originalEmail.current,
        {
          name,
          contactEmail: email,
          phoneNumber: phone,
      ...
      setContacts(
        contacts.map((contact) =>
          contact.email === email ? response.data.msg : contact
        )
      );
      ...
      <div className="modal-box bg-white">
        ...
        <div className="flex justify-between items-center">
          <div className="modal-action ml-auto">
            ...
            <button
              onClick={action === "create" ? addContact : updateContact}
...
export default MaintainContactModal;
